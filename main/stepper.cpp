...
void stepper_init()
{
	...
	gpio_set_direction(X_DIRECTION_PIN, GPIO_MODE_OUTPUT);
#endif
#ifdef Y_DIRECTION_PIN
	gpio_set_direction(Y_DIRECTION_PIN, GPIO_MODE_OUTPUT);
#endif
	...
	gpio_set_direction(STEPPERS_DISABLE_PIN, GPIO_MODE_OUTPUT);
#endif
...
#ifdef X_DIRECTION_PIN
	gpio_set_level(X_DIRECTION_PIN, (onMask & (1 << X_AXIS)));
#endif
#ifdef Y_DIRECTION_PIN
	gpio_set_level(Y_DIRECTION_PIN, (onMask & (1 << Y_AXIS)));
#endif
}
...
#ifdef X_STEP_PIN
	gpio_set_level(X_STEP_PIN, (onMask & (1 << X_AXIS)));
#endif
#ifdef Y_STEP_PIN
	gpio_set_level(Y_STEP_PIN, (onMask & (1 << Y_AXIS)));
#endif
}
...
void st_go_idle()
{
	...
	if (((settings.stepper_idle_lock_time != 0xff) || sys_rt_exec_alarm || sys.state == STATE_SLEEP) && sys.state != STATE_HOMING)
	{
		...
		stepper_idle_counter = esp_timer_get_time() + (long long int) (settings.stepper_idle_lock_time * 1000); // * 1000 because the time is in uSecs
...
#ifdef STEPPERS_DISABLE_PIN
	gpio_set_level(STEPPERS_DISABLE_PIN, isOn);
#endif
}
